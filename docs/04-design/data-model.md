...
| source_type | Enum/String | Yes | Payment, settlement, internal, etc. |
| status | Enum | Yes | Active or inactive |
| created_at | Timestamp | Yes | Creation timestamp |
| updated_at  | Timestamp | Yes | Last update timestamp |
...
| external_transaction_id | String | No | Source-specific transaction identifier |
...
| transaction_type | Enum | Yes | Payment, settlement, refund, etc. |
...
| exception_type | Enum | Yes | Amount mismatch, currency mismatch, duplicate, etc. |
...
2. Which identifier format will be used for primary keys?
3. Should monetary values remain integer minor units or use an exact decimal database type?
4. Should original source payloads be retained?
5. If raw payloads are retained, how should they be structured and indexed?
6. Can one transaction participate in multiple successful reconciliation results?
7. How should superseded or historical results be identified?
8. Should resolution history support multiple resolutions if an exception is reopened?
9. How should validation failures be persisted?
10. Should reconciliation status exist directly on Transaction, or be derived from reconciliation results?
11. Which source identifiers are sufficiently reliable to support database uniqueness constraints?
12. How should concurrent workers coordinate access to the same reconciliation candidates?
